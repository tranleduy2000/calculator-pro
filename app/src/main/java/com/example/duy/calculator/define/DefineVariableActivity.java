...
public class DefineVariableActivity extends AbstractAppCompatActivity {
    ...
    private ArrayList<String> getIntentData() {
        ...
        try {
            ...
            ArrayList<String> variables = BigEvaluator.getInstance(this).getListVariables(expr);
    ...
    private void doSave() {
        ...
        for (VariableEntry e : entries) {
            ...
            if (BigEvaluator.getInstance(this).isNumber(e.getValue()) && !e.getName().isEmpty()) {
                BigEvaluator.getInstance(this).define(e.getName(), e.getValue());
