...
//       fix bug -> mMathView.setText(""); -> mReview
    ...
    public void clickFactorPrime() {
        ...
        BigEvaluator.getInstance(getApplicationContext()).factorPrime(mInputDisplay.getCleanText(),
...
//            Log.d(TAG, "doInBackground: " + item.getInput());
            ...
            if (BigEvaluator.getInstance(getApplicationContext()).isSyntaxError(item.getLeftExpr())) {
                return "$$" + BigEvaluator.getInstance(getApplicationContext()).getError(item.getLeftExpr()) + "$$";
            }

            if (BigEvaluator.getInstance(getApplicationContext()).isSyntaxError(item.getRightExpr())) {
                return "$$" + BigEvaluator.getInstance(getApplicationContext()).getError(item.getRightExpr()) + "$$";
            }
